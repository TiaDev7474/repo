...
import {getTokenConnect} from "@/actions/actions";

const  {CLIENT_SECRET,CLIENT_ID} = process.env
const AuthCityZen = () => {
  const session = useSession();
  const handleOnClick = async () => {
       await getTokenConnect(CLIENT_ID!, CLIENT_SECRET!)
  }
  return (
      ...
      <div className="bg-white p-10 rounded-3xl shadow-md flex flex-col items-center">
        ...
        <div className="w-full flex items-center">
          ...
          <Button
            ...
            className="flex items-center w-full pt-2 pb-2 pr-5 py-10  pl-5 outline-none hover:outline-0 rounded-2xl hover:bg-gray-100"
          >
            ...
            <div className="text-start ml-4">
              <h3 className="text-lg font-semibold">
                {session?.data?.user?.name || "Nomena "}
              </h3>
              <p className="text-gray-500 font-semibold opacity-45 text-[15px]">
                {session?.data?.user?.email || "Andrianiaina Fanomezantsoa"}
              </p>
            </div>
          </Button>
        </div>
